...
충돌을 해결하기 위한 복제 토폴로지(쓰기가 전달되는 통신 경로)는 아래와 같다.
1. 원형 토폴로지 - 다른 노드로부터 쓰기를 전달받고 자신의 쓰기를 합해서 다음 노드로 전달한다.
2. 성형 토폴로지 - 지정된 루트 노드 하나가 다른 모든 노드에 쓰기를 전달받고 또 전달한다.
3. 전체 연결 토폴로지 - 모든 리더가 다른 리더들에게 쓰기를 전송한다.

하지만 토폴로지를 설정한다고 해서 모든 이벤트가 순서대로 작동할 것이라고 예상할 수는 없다.(네트워크 환경에 의해 미래의 이벤트가 과거의 이벤트를 추월할 수도 있다.)
...
일부 DB는 리더가 아닌 모든 복제 서버가 클라이언트로부터 쓰기를 직접 받을 수 있게 허용한다.
이와 같은 종류의 데이터베이스를 다이나모 스타일이라고 한다.

클라이언트가 모든 복제 서버에 요청을 전달하도록 하기도 한다.
만약 어떤 클라이언트가 요청을 받아들이지 못했다면 (조건에 맞는 경우) 클라이언트는 이를 무시한다.
이후 해당 데이터를 불러올 때, 가장 최신 데이터를 버젼 넘버를 사용하여 결정한다.
```

### Recovery
```
Replication은 모든 복제 서버에 복사가 된 것을 보장해야 한다. 다이나모 스타일 데이터베이스에서는 사용 불가능했던 노드가 온라인 상태가 된 후 누락된 쓰기를 따라잡기 위해 아래 두 가지 메커니즘을 주로 사용한다.:

* 읽기 복구 - 여러 노드에서 병렬로 읽기를 수행하고, outdated된 값을 반환한 노드에 최신값 쓰기를 실행한다. 읽기가 많은 유즈케이스에 적합하다.

* 안티 엔트로피 처리 - 백그라운드 프로세스가 복제 서버사이의 데이터 차이를 찾아 누락된 데이터를 복사한다. 복사가 이루어지기까지 상당한 지연이 있을 수 있다.
```

### Threshold
```
Leaderless에서 모든 성공한 쓰기가 적어도 두 개 이상의 복제 서버에 존재한다는 것을 보장한다면 하나의 복제 서버가 오래된 데이터임을 알 수 있다. 따라서 적어도 두 개의 복제 서버에서 읽으면 두개 중 하나는 최신값임을 확신할 수 있다.

일반적으로 n개의 복제 서버가 있을 때(주로 홀수), 쓰기 성공 확정을 위해서는 w개의 노드에서 쓰기가 성공해야하며, 이는 n과 같은 r개의 노드에 읽기 요청을 보내는 것과 같다. 이때 r == n == round((n+1)/2)로 정하는 것이 일반적이다.

General Rule of Thumb은 w + r > n 이면 읽기 요청을 보냈을 때 최신값을 반환한다고 생각할 수 있다. (읽기와 쓰기에서 겹치는 노드가 최소 1개 이상 있다)

만약 네트워크 문제로 w개의 노드에 쓰기를 요청할 수 없는 경우에는 1. 모든 쓰기 요청에 오류를 반환하거나 2. 일단 쓰기를 받아들이고 연결된 노드에만 이를 요청한다 와 같은 해결책이 있다.
...
## Partitioning
한 노드에 할당할 수 있는 것보다 더 많은 데이터를 저장하기 위해서 데이터를 특정 키로 분류해서 서로 다른 노드에 저장하는 것을 파티션 (ES, MongoDB 등에서는 Shard)라고 한다.\
이론적으로 N개의 노드를 가진 클러스터는 N배의 데이터를 저장할 수 있다.(고가용성을 위한 복제 등으로 실제로는 N배보다 적다.)

```
한 노드가 여러 파티션을 가질 수 있다.
예를 들어 노드 1은 파티션 1의 리더이면서 파티션 2와 3의 팔로워일 수도 있다.
파티션으로 나누는 기준으로 주로 사용되는 것은 아래와 같다:
1. Range Partition: 데이터의 column값을 파티션 키로 사용한다. Range Query에 유용하지만 어떤 한 파티션이 보틀넥이 되는 현상이 발생하기 쉬우므로 최근에는 잘 사용되지 않는다.
2. Hash: Hash값을 사용해서 무작위하게 / 혹은 모든 파티션이 골고루 사용되도록 나눌 수 있다.
```
